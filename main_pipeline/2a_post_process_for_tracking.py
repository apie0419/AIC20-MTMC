...
class Box(object):
    ...
    def __init__(self, frame_index, id, box, score, gps_coor, feature):
        ...
        self.center = (int(self.box[0] + self.box[2] / 2), int(self.box[1] + self.box[3] / 2))
        self.match_state = NO_MATCHED
        self.floor_center = (int(self.box[0] + self.box[2] / 2), int(self.box[1] + self.box[3] * 0.8))
...
def analysis_time_stamp():
    ...
    for filename in filenames:
        lines = open(os.path.join(time_stamp_dir, filename), "r").readlines()
        for line in lines:
            words = line.strip('\n').split()
            camera_id = words[0]
