    ...
    def merge(self, tk_gp):
        ...
        for i in cur_ori:
            if i not in self.orientations:
                self.orientations = self.orientations + i
        # 修改移动向量
        # subscriber_mv = tk_gp.gps_move_vec*10000
        # master_mv = self.gps_move_vec*10000
        # subscriber_dis = subscriber_mv[0] ** 2 + subscriber_mv[1] ** 2
        # master_dis = master_mv[0] ** 2 + master_mv[1] ** 2
        # if subscriber_dis > master_dis:
        #     # print 'move vec changed: ', master_mv, ' to ', subscriber_mv
        #     self.gps_move_vec = tk_gp.gps_move_vec
...
def fuse_track_groups_strict(track_groups_list, near_th):
    ...
    for cur_tk_gp in track_groups_list:
        ...
        for res_tk_gp in result_list:
            ...
            similar_th = near_th
            if cameras_diff(res_tk_gp, cur_tk_gp) and tkgp_same_direction(res_tk_gp, cur_tk_gp):
                dis = tkgp_feature_distance(res_tk_gp, cur_tk_gp)
                # print dis
                if dis < similar_th:
                    res_tk_gp.merge(cur_tk_gp)
    ...
    for cur_tk_gp in track_groups_list:
        ...
        if cur_tk_gp.get_length() == 1:
            for res_tk_gp in result_list:
                ...
                if cameras_diff(res_tk_gp, cur_tk_gp) and tkgp_same_direction(res_tk_gp, cur_tk_gp):
                    dis = tkgp_feature_distance(res_tk_gp, cur_tk_gp)
                    # print "***", dis, similar_th
                    if dis < similar_th:
                        # print 'same dirciton:', dis
                        res_tk_gp.merge(cur_tk_gp)
                        creat_new_tk_gp = False
                        break
            # 一条都未匹配上，形成新tk，加入res
